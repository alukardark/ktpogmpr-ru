...
<?else:?>
	...
	<div class="modal-body">
		<form action="<?=POST_FORM_ACTION_URI?>" method="POST" enctype="multipart/form-data" class="form">
			<?=bitrix_sessid_post()?>
			...
			<fieldset class="form-group <?if($arResult["ERROR_MESSAGE"]["EMAIL"]):?>has-error<?endif?>">
				<label for="form-email"><i class="fa fa-envelope"></i> <?=GetMessage("MFT_EMAIL")?></label><br>
				<input id="form-email" type="text" name="user_email" class="form-control input-sm" value="<?=$arResult["AUTHOR_EMAIL"]?>">
			</fieldset>

			<?if($_GET["subject"]):?>
					<input id="form-subject" type="hidden" value="<?=iconv('utf-8', LANG_CHARSET, htmlspecialchars($_GET["subject"]))?>" name="subject">
			<?endif?>

			<div class="form-group">
				<a class='btn btn-sm btn-dark upload-button' href='javascript:;'>
					<?=GetMessage("SELECT_FILE")?>
					<input id="form-attach" type="file" value="<?=$arResult["ATTACH_FILE"]?>" name="attach_file" size="40"  onchange='fileUploadSetName(this);'>
				</a>
				&nbsp;
				<span class="badge" id="upload-file-info"></span>
			</div>

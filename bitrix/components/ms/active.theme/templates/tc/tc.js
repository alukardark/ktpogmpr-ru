$(document).ready(function(){
	...
	// 	var target = $(el).closest('.tc_section');
	// 	if(target.attr('id')){
	// 		$.cookie("tc_latest", target.attr('id'),{path: '/'});
	// 	}
	// }
	//
	// if($.cookie("tc_latest")){
	// 	var tcLatest = $('#' + $.cookie("tc_latest"));
	//
	// 	if(tcLatest.length){
	// 		$('#theme_changer_scroll_container').scrollTop( $('#' + $.cookie("tc_latest")).position().top + 6);
	// 	}
	//
	// 	$.removeCookie('tc_latest', { path: '/' });
	// }
	function rememberLatest(el){
		$.cookie("tc_latest", $('#theme_changer_scroll_container').scrollTop(),{path: '/'});
	}

	if($.cookie("tc_latest")){
		var tcLatest = parseInt($.cookie("tc_latest"));

		if(tcLatest){
			$('#theme_changer_scroll_container').scrollTop(tcLatest);
		}

		...
		<table class="search-page-form">
			<tbody><tr>
				<td style="width: 100%;">
					...
					<?else:?>
						<input class="form-control search-query" type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" />
					<?endif;?>
				</td>
				<td>
					<input class="btn btn-primary btn-bold search-button" type="submit" value="<?echo GetMessage("CT_BSP_GO")?>" />
				</td>
			</tr>
		</tbody></table>
		...
		<div class="search-advanced">
			...
			<?if($arParams["SHOW_WHERE"] || $arParams["SHOW_WHEN"]):?>
				...
				<div id="search_params" class="search-filter" style="display:<?echo $arResult["REQUEST"]["FROM"] || $arResult["REQUEST"]["TO"] || $arResult["REQUEST"]["WHERE"]? 'block': 'none'?>">
					...
					<table class="table search-filter" cellspacing="0"><tbody>
						<?if($arParams["SHOW_WHERE"]):?>
						<tr>
							<td class="search-filter-name"><?echo GetMessage("CT_BSP_WHERE")?></td>
							<td class="search-filter-field">
								<select class="form-control input-sm select-field" name="where">
						...
						<tr>
							...
							<td class="search-filter-field"><input class="btn btn-primary btn-bold search-button" value="<?echo GetMessage("CT_BSP_GO")?>" type="submit"></td>
						</tr>
					</tbody></table>
				</div>
			<?else:?>

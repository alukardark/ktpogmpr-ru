...
<?
	if (true === $arResult["bDescPageNumbering"])
	{
		...
		{
			?><a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>" title="<? echo GetMessage('nav_prev_title'); ?>"><i class="fa fa-angle-left"></i></a><?
		}
		else
		{
		...
		while ($NavRecordGroup >= 1)
		{
			...
			{
				?><li class="active" title="<? echo GetMessage('nav_page_current_title'); ?>"><span><? echo $NavRecordGroupPrint; ?></span></li><?
			}
		...
		{
			?><a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>" title="<? echo GetMessage('nav_next_title'); ?>"><i class="fa fa-angle-right"></i></a><?
		}
		else
		{
...
?>
		...
		while($NavRecordGroup <= $arResult["NavPageCount"])
		{
			...
			if ($NavRecordGroup == $arResult["NavPageNomer"])
			{
				?><li class="active" title="<? echo GetMessage('nav_page_current_title'); ?>"><span><? echo $NavRecordGroup; ?></span></li><?
			}
